const {MaintenanceWorkflowCommandService, MemoryWorkflowStore, at, persistedTimestamp, seedActor, seedFurnaceHierarchy, seedInstalledInnerCoverHierarchy, definition, upsertDefinition, createCampaign, deleteUnusedCampaign, observation} = require('./helpers/inspectionFixture');
...
describe('cross-asset inspection campaigns', () => {
  ...
  test('blocks silent partial closure and links a finding only to the same asset', async () => {
    ...
    store.seed('maintenance_records/ticket-other', {
      ...
      assetHierarchyRefJson: JSON.stringify({schemaVersion: 3, scope: 'physicalAsset', assetClassId: 'class-furnace', assetInstanceId: 'furnace-2', assetNumber: 2, assetInstanceVersion: 1}),
      isDeleted: false,
    });
