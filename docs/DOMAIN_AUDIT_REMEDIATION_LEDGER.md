...
| D07-04 inspection uncertainty is lost in the management snapshot | **repaired** | My earlier note called this the same root as D05-02. It is not, and reading the dossier entry properly made it tractable. An inspection finding whose only adverse reading was later corrected keeps three markers saying technical verification cannot substitute for reviewing the corrected basis. The Morning Review compiler built a narrower fact and dropped all three, so the frozen minutes showed an ordinary item awaiting verification while the originating module demanded a different adjudication first. The fact carries them now, the summary says 'Evidence review required before verification' instead of a recurrence count that a correction leaves standing, and the client reader accepts them as optional so records written before the qualifier still read while a present but malformed value still fails closed. The shared contract fixture that both the Jest and the Flutter test decode was widened with them. |
...
| R-06 retired subjects and definitions lack a historical-entry or amendment route (11) | **owner decision** | Checked: the refusal is real and the audit calls it a policy gap, not a source defect. It is the correction question in its eleventh-domain form and is closed to this pass with the other six. |
