...
## Pending follow-up recorded by the owner â€” 20 September 2026

- **UV installation-date correction â€” pending, not implemented.** Add an explicit correction route that preserves the original installation record and provides reviewed, auditable correction history and recovery. Ordinary replacement entry must not be used to disguise a date correction.
- **Coordinated client/backend rollout â€” pending release prerequisite.** Before activating the new Burner/UV provenance producers and partial editor, deliver compatible readers and verify that older incompatible clients are upgraded or gated. Source push does not establish deployment readiness.

These items were explicitly retained at the owner's request after commit `d038797f`. The latest Burner/UV repair status is in [the 20 September remediation record](BURNER_UV_REVIEW_REMEDIATION_2026_09_20.md); it supersedes the earlier Domain 04 progress notes below. Recording these items does not start implementation or deployment.

What the three domain audits of 14 September 2026 found, what has been repaired
in the backend, and what is deliberately left â€” with the reason, so the next
...
- **repaired** â€” fixed here, with a regression test that fails without the fix.
- **client** â€” the defect is in the Flutter or native client. It can be written
  at any time, but it only reaches operators in a new build, and no build is
  authorised.
- **decision** â€” the repair requires a business decision that code review cannot
  invent. The options are stated; the owner is the plant, not the implementer.
- **design** â€” a bounded repair does not exist; the domain needs a model it does
  not have yet (an evidence episode, an accountable exception owner).

## Domain 02 â€” Multi-agency planned maintenance
...
## Domain 01 â€” Critical Safety Alarms
...
| CSA-07 one malformed active record can interrupt the whole active feed | **client** | A whole snapshot is decoded through one throwing map, so a single damaged record can end the stream. Needs per-record outcomes and an explicit completeness state â€” the same shape as the quality feed isolation item. |
...
## Domain 03 â€” Inner Cover lifecycle
...
| D03-01 old inspection evidence can qualify a cover after later repair or retirement | **partly repaired** | Filed with the correction family, wrongly again â€” and this one is an assurance defect, not a records question. A cover accepted on 2 August, removed into repair on the 8th or retired as bulged, could be re-accepted on the 11th using the same 2 August inspection and installed under a Base. The only lower bound on the inspection date was the original receipt. **Decision: a re-acceptance cannot rest on the evidence the previous one already used.** An inspection dated at or before the acceptance it replaces certifies nothing about what happened since, so it is refused. The comparison is between two physical inspection dates, never recording times, so evidence entered late is unaffected. Three existing tests re-accepted returned covers on the original inspection â€” the audit names one of them as revealing â€” and they now date the re-inspection to the return that prompted it. What remains is the fuller model the audit describes: a recorded assurance episode with the physical event that invalidated the previous clearance, which would also refuse an inspection dated after the last acceptance but before the damage. That needs a physical event time the lifecycle does not record today. |
...
## Domain 04 â€” Burner blocks, UV detectors and condition rounds
...
## Domain 08 â€” Reactive maintenance
...
## Domains 11 to 18 â€” the remaining business functions
...
I grouped seven findings here as one question â€” what it means to correct a
record the plant has already acted on, and who may do it â€” and treated the whole
...
- It distinguishes the four cases the sixth domain names: a duplicate recording,
  a withdrawn erroneous entry, a corrected value, and a true recurrence â€” which
  already works and must not be the workaround for the other three.

Why it is not built here: each instance is a full governed command â€” authority,
audit, receipt, replay derivation, consumer recomputation â€” and the value of
...
Three of the repairs above â€” D02-01, D06-01 and D10-01 â€” are the same mistake in
...
that asset was reported as damaged data â€” `asset-instance-open-condition-ticket-malformed`
â€” instead of as the open condition that genuinely stands in the way of
...
|---|---|
| D04-02 carrying an observation forward with its own time, observer and evidence episode | a sparse compliance event, or field-level provenance on a round. Today a round is eight positions or nothing, which is why compliance had to copy values it did not observe. The worst of it â€” a round recorded before UV and draft-seal evidence existed being read as eight serviceable positions â€” is repaired. |
...
## Function 11 follow-up â€” 20 September 2026

See `MAINTENANCE_CADENCE_REMEDIATION_2026_09_20.md`. Shared reduction, same-day conflict qualification, persisted historical-only exclusion, original-actor attribution, history revision grouping and verified plan basis were repaired locally. Legacy overlaps are contained, not merged. Historical correction/adjudication, reviewed migration, full retirement integration and native/release verification remain open. This supersedes any blanket suggestion that all remaining work is merely owner-blocked or optional product enhancement.


## Function 12 follow-up â€” 20 September 2026

See `CHARGE_MONITORING_REMEDIATION_2026_09_20.md`. Charge-monitoring legacy replay, lazy native dependencies, durable recovery, owner-approved context correction and distinct cancellation, qualified/monotonic populations, closure replay integrity, uncapped legacy Base certification and starvation-resistant archival were repaired or confirmed with current-tree regressions. Reader-first schema-4 rollout, deployed recovery/fence activation, device/emulator/browser validation and exact release gates remain required. Terminal reopening/supersession and structured observations were not introduced. No commit, push, artifact or production change was made by this Function 12 pass.


## Function 13 follow-up â€” 20 September 2026

See `ORDINARY_DIRECTIVES_REMEDIATION_2026_09_20.md`. Ordinary directives now use immutable original-actor commands, atomic audit/acceptance, exact reviewed-basis checks, fresh acknowledgement after active amendment/transfer, guarded recovery, canonical readback and qualified partial populations. Arbitrary offline action chaining is deliberately contained until predecessor confirmation. Older collapsed/unbound work is preserved for reviewed reconciliation. Deployment/Rules coordination, explicit recovery activation, mixed-version qualification and device/browser release evidence remain required. This pass did not commit, push, deploy or build a distribution artifact.

## User administration (Function 15 / PDF Function 14) - 20 September 2026

See `USER_ADMINISTRATION_REMEDIATION_2026_09_20.md`. Current access withdrawal remains reversible through a reviewed Admin restoration. Monotonic authority revisions, outcome-only original-actor recovery, native durable ownership, qualified directory reads, protected access disposition, stronger audit binding and notification-generation safeguards were implemented or completed and tested. The owner explicitly chose an online access check before opening; startup/foreground gating preserves saved work. Deployment coordination, exact-device/upgrade checks, reviewed repair of any damaged historical profiles, browser durable write support and supervised unavailable-account/off-device recovery remain open. No commit, push, production change or distribution artifact was made in this pass.


## Functions 14/16 and remaining source audit - 20 September 2026

See `REMAINING_FUNCTIONS_REMEDIATION_2026_09_20.md` for the finding-by-finding disposition. Added native durable registry recovery and immutable historical installation correction; repaired report selection, provenance, plant periods and current retained follow-up; strengthened knowledge import/adoption/audit and diary concurrency/late follow-up; fenced purged identities and retained notification exceptions. Owner decisions permit physical closure with visible diary follow-up and retain administratively closed still-relevant concerns until explicit relevance end. Report source isolation/reproducibility, knowledge edition certification, ordinary per-recipient notification recovery and registry browser/off-device recovery remain open. These are not all owner-blocked decisions. Current-source inventory refresh does not renew historical production approval or certify a new release.
