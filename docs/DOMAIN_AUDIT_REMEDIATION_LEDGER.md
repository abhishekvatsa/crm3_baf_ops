...
| D02-04 removed or terminated lanes veto already prepared RED work | open | `prepareRedLane` reads the active set; `acknowledgeLane` reads every generation, so superseded history blocks work that preparation already cleared. Bounded: both must use one validated active-generation set. |
...
| D02-06 preselected RED stand preparation has no initial escalation schedule | open | The produced handover carries `raisedAt` and `becameDueAt` but neither `acknowledgementDueAt` nor `nextEscalationAt`, and the sweeper queries the latter, so this one request never enters the initial escalation path. Bounded, but it also asks whether the blocked agency's clock keeps running while Operations prepares — state that with the fix. |
