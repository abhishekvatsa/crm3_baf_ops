...
void main() {
  group('R1.14 Firestore expression-budget contract', () {
    ...
    test('authorization reads require a canonical security capsule', () {
      ...
      expect(
        authority,
        contains("data.keys().hasAll(['isApproved', 'roles'])"),
      );
    ...
    test(
      'job execution completion is rejected before expensive work validation',
      () {
        ...
        expect(
          update,
          contains("request.resource.data.get('isCompleted', false) !="),
        );
    ...
    test('maintenance has one update allow and a transition router', () {
      ...
      expect(
        ...
        1,
        reason:
            'overlapping update allows consumed the emulator expression budget',
      );
      expect(
        matchBlock,
    ...
    test(
      'template lifecycle evaluates only the selected status transition',
      () {
        final router = _blockStartingAt(
          rules,
    ...
    test(
      'job-module updates validate one user snapshot and one lifecycle branch',
      () {
        final entry = _blockStartingAt(
          rules,
    ...
    test('directives snapshot roles and route one transition branch', () {
      ...
      expect(
        rules,
        isNot(contains('function canDirectiveCreatorTarget(role)')),
      );
      expect(
        rules,
        isNot(contains('function hasRequesterDirectiveTargetRole(role)')),
      );

      final update = _blockStartingAt(rules, 'function validDirectiveUpdate()');
      expect(update, contains("userDoc().data.get('roles', [])"));
      expect(update, contains('validDirectiveUpdateForRoles'));
    });

    test(
      'module registry uses status routers and one family read per publish',
      () {
        ...
        expect(
          family,
          contains("request.resource.data.get('status', null) == 'active'"),
        );
        expect(
          family,
          contains('validModuleRegistryFamilyPublishDelta(docId)'),
        );
