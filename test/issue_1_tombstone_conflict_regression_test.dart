...
void main() {
  ...
  group('Issue 1: remote tombstones preserve fresher unsynced local evidence', () {
    test('maintenance ticket tombstone preserves newer dirty local ticket', () async {
      await _withTestIsar((isar) async {
        ...
        expect(result.outcome, RemoteTombstoneApplyOutcome.localDirtyPreserved);
        final after = await isar.maintenanceRecords
            .filter()
    ...
    test('job execution tombstone preserves newer dirty local execution', () async {
      await _withTestIsar((isar) async {
        final localUpdatedAt = DateTime.utc(2026, 5, 12, 10, 10);
        final remoteDeletedAt = DateTime.utc(2026, 5, 12, 10, 0);
        await isar.writeTxn(() => isar.jobExecutions.put(_jobExecution(
        ...
        expect(result.outcome, RemoteTombstoneApplyOutcome.localDirtyPreserved);
        final after = await isar.jobExecutions
            .filter()
    ...
    test('job module tombstone preserves newer dirty local module responses', () async {
      await _withTestIsar((isar) async {
        final localUpdatedAt = DateTime.utc(2026, 5, 12, 10, 10);
        final remoteDeletedAt = DateTime.utc(2026, 5, 12, 10, 0);
        await isar.writeTxn(() => isar.jobModuleInstances.put(_jobModule(
        ...
        final result = await IsarJobModuleRepository().applyTombstoneFromRemote(remote);

        expect(result.outcome, RemoteTombstoneApplyOutcome.localDirtyPreserved);
        final after = await isar.jobModuleInstances
            .filter()
    ...
    test('job diary tombstone preserves newer dirty local diary note', () async {
      await _withTestIsar((isar) async {
        final localUpdatedAt = DateTime.utc(2026, 5, 12, 10, 10);
        final remoteDeletedAt = DateTime.utc(2026, 5, 12, 10, 0);
        await isar.writeTxn(() => isar.jobDiaryEntrys.put(_diaryEntry(
          firestoreId: 'diary_1',
          createdAt: DateTime.utc(2026, 5, 12, 9),
          updatedAt: localUpdatedAt,
          isSynced: false,
        ...
        final result = await IsarJobDiaryRepository().applyTombstoneFromRemote(remote);

        expect(result.outcome, RemoteTombstoneApplyOutcome.localDirtyPreserved);
        final after = await isar.jobDiaryEntrys
            .filter()
  ...
  group('Issue 1: remote tombstones still apply when local row is clean or older', () {
    test('maintenance tombstone applies to clean older local ticket', () async {
      await _withTestIsar((isar) async {
        final localUpdatedAt = DateTime.utc(2026, 5, 12, 9, 55);
        final remoteDeletedAt = DateTime.utc(2026, 5, 12, 10, 0);
        await isar.writeTxn(() => isar.maintenanceRecords.put(_maintenanceRecord(
