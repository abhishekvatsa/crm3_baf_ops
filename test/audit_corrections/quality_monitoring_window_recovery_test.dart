...
void main() {
  testWidgets(
    'expiry timer cannot restore success after a required source fails',
    (tester) async {
      ...
      final request = QualityMonitoringRequest(
        requestId: 'expiry-fixture',
        baseNumber: 205,
        grade: 'TEST',
        cycleReference: 'test-only',
        chargeNumbers: const <int>[12345],
        ...
        visibleUntil: now.add(const Duration(seconds: 1)),
        archivedAt: null,
        createdAt: now,
      ...
      legacy.add(<QualityMonitoringRequest>[request]);
      await tester.pump();
      ...
      expect(
        values,
        hasLength(1),
        reason: 'expiry is not a fresh read of the failed source',
      );
