...
void main() {
  group('Morning Review agenda compiler', () {
    ...
    test('retains carried action identity and age in today discussion', () {
      ...
      expect(matter.categories, contains(MorningReviewAgendaFilter.resolved));
      expect(
        matter.categories,
        isNot(contains(MorningReviewAgendaFilter.open)),
      );
    });
    ...
    test('classifies a withdrawn-in-error alarm as resolved', () {
      final agenda = compileMorningReviewAgenda(
        sourceFacts: [
          _fact(
    ...
    test('classifies native quality and inspection lifecycle states', () {
      ...
      expect(agenda.countFor(MorningReviewAgendaFilter.open), 2);
      expect(agenda.countFor(MorningReviewAgendaFilter.resolved), 2);
    });
  });
}
