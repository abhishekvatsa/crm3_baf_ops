...
void main() {
  ...
  late BurnerBlockCorrectionCommandService service;
  ...
  Future<void> open() async {
    database = await Isar.open(
      ...
      name: 'burner_correction_recovery',
      directory: directory.path,
      inspector: false,
    );
    journal = DurableSubmissionRepository(database);
    service = BurnerBlockCorrectionCommandService(
      gateway: gateway,
  ...
  setUp(() async {
    directory = await Directory.systemTemp.createTemp('burner_correction_');
    actor = 'admin-a';
    gateway = _Gateway();
  ...
  Future<WorkflowCommandReceipt> executeOriginBoundEnvelope(
    ...
    return WorkflowCommandReceipt(
      ...
      result: {
        ...
        'auditId': 'server_burner_block_correction_${command['commandId']}',
