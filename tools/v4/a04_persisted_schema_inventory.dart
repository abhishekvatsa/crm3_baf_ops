...
String _decoderContract(String policy) {
  ...
  if (policy == 'durable-submission-receipt-v1') {
    return 'A domain validator binds acceptance to the exact frozen operation inside settleAccepted; native hash/state integrity and domain revalidation precede adoption. Only replay-observation flags are canonicalized. Distinct typed administrative review proof binds the original immutable evidence hash, reviewer, reason and server decision with acceptedAt null; original unknown actor and source bytes remain unchanged. A matching validated late acceptance retains both histories in a typed acceptedPendingAdoption capsule; cancellation or contradictory evidence stays reviewConflict. The current view exposes the canonical domain receipt/hash and separate retained review history; older v12 readers preserve accepted/unresolved evidence and refuse unsupported adoption. Current domain readback remains required before reconciliation.';
  }
