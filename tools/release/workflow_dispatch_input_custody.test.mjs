...
test('C-01 governance remains source-only until merge evidence exists', () => {
  ...
  assert.equal(finding[0].currentStatus, 'SOURCE_IMPLEMENTED');
  assert.deepEqual(
    finding[0].statusHistory.map((entry) => entry.status),
    ['OPEN', 'SOURCE_IMPLEMENTED'],
  );
  assert.equal(finding[0].evidence.length, 0);
  ...
  assert.match(decision, /Status: SOURCE_IMPLEMENTED/);
