...
import '../../providers/uv_detector_correction_provider.dart';
...
class UvDetectorCorrectionControls extends StatelessWidget {
  const UvDetectorCorrectionControls({super.key, required this.event});
  final UvDetectorLifecycleEvent event;
...
class _CorrectionDialogState extends ConsumerState<_CorrectionDialog> {
  ...
  UvDetectorCorrectionReview? _review;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      _requireOrigin();
      final service = ref.read(uvDetectorCorrectionCommandServiceProvider);
    ...
    try {
      ...
      if (_pending != null) {
        ...
        await service.correct(
          ...
          expectedCurrentActionPerformedAt: review.current.actionPerformedAt
              .toUtc()
              .toIso8601String(),
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text('UV ${widget.event.burnerPosition} installation correction'),
      content: SizedBox(
        width: 520,
                ...
                if (pending != null) ...[
                  ...
                  if (review.corrections.isNotEmpty)
                    ExpansionTile(
                      ...
                      children: [
                        for (final correction in review.correctionHistory)
                          ListTile(
                            title: Text(
                              'Effective installation: ${_date.format(correction.effectiveAt.toLocal())}',
                            ),
                            subtitle: Text(
                              '${correction.reason}\nReviewed by ${correction.reviewerName} on ${_date.format(correction.correctedAt.toLocal())}',
                            ),
