...
class OperationsReportPdfService {
  ...
  static pw.Widget _documentIdentity({
    ...
    children: <pw.Widget>[
        ...
        children: <pw.Widget>[
          pw.Expanded(
            child: _metadataGrid(<String, String>{
              'Reporting period':
                  '${_date.format(report.filter.startDate)} to ${_date.format(report.filter.endDate)}',
              'Calendar zone': operationsReportTimeZone,
              'UTC interval [start, end)':
                  '${report.filter.startInclusive.toIso8601String()} / ${report.filter.endExclusive.toIso8601String()}',
              'Asset class': assetClassLabel,
              'Asset': assetLabel,
              'Report evaluated at': _dateTime.format(
                operationsReportPlantTime(report.asOf),
              ),
            }),
          ),
          ...
          pw.Expanded(
            child: _metadataGrid(<String, String>{
              ...
              'Generated at': _dateTime.format(
                operationsReportPlantTime(request.generatedAt),
              ),
      ...
      pw.SizedBox(height: 12),
      if (report.unreadableDueStateCount > 0)
        _incompleteEvidenceStatement(
          '${report.unreadableDueStateCount} maintenance due-state records could not be read. '
          'Cadence totals cover readable records only; zero does not establish that no maintenance is due.',
        ),
      pw.Container(
  ...
  static List<pw.Widget> _executiveSummary(
    ...
    _sectionHeading(
      'Executive summary',
      'Selected-period outcomes with current exception and availability signals; current signals are not historical-period totals.',
    ),
      ...
      if (tickets.isEmpty)
        ...
        ..._simpleTables(
          ...
          rows: tickets
              .map(
                (ticket) => <String>[
                  _dateTime.format(operationsReportPlantTime(ticket.startDate)),
                  ticket.endDate == null
                      ? '-'
                      : _dateTime.format(
                          operationsReportPlantTime(ticket.endDate!),
                        ),
  ...
  static List<pw.Widget> _plannedMaintenance(OperationsReport report) {
    ...
    return <pw.Widget>[
      ...
      pw.SizedBox(height: 10),
      if (report.unreadableExecutionCount > 0)
        _incompleteEvidenceStatement(
          'Planned-work evidence is incomplete: '
          '${report.unreadableExecutionCount} source '
          '${report.unreadableExecutionCount == 1 ? 'record could' : 'records could'} '
          'not be decoded. Completion is not presented as a complete-population percentage.',
        ),
      if (jobs.isEmpty)
        _emptyStatement(
      ...
      if (report.dueStates.isEmpty)
        ...
        ..._simpleTables(
          ...
          rows:
              (report.dueStates.toList(growable: false)..sort((left, right) {
                  ...
                  .map(
                    (state) => <String>[
                      ...
                      state.nextDueAt == null
                          ? 'Not calculated'
                          : _dateTime.format(
                              operationsReportPlantTime(state.nextDueAt!),
                            ),
        ...
        rows: report.directives
            .map(
              (directive) => <String>[
                _dateTime.format(
                  operationsReportPlantTime(
                    (directive.issuedAt ?? directive.createdAt),
                  ),
                ),
                ...
                directive.acknowledgedAt == null
                    ...
                    : '${_reportLabel(directive.status.name)} at '
                          '${_dateTime.format(operationsReportPlantTime(directive.acknowledgedAt!))} by '
            ...
            .map(
              (lane) => <String>[
                ...
                lane.acknowledgedAt == null
                    ? 'Not acknowledged'
                    : '${_dateTime.format(operationsReportPlantTime(lane.acknowledgedAt!))} by '
                          '${lane.acknowledgedByName ?? 'Not recorded'}',
                lane.closedAt != null
                    ? '${_dateTime.format(operationsReportPlantTime(lane.closedAt!))} by '
            ...
            .map(
              (request) => <String>[
                '${_reportLabel(request.assetTypeKey)} ${request.assetNumber}\n'
                    '${_dateTime.format(operationsReportPlantTime((request.raisedAt ?? request.createdAt)))}',
              ...
              (warning) => <String>[
                ...
                warning.closureRequestedAt == null
                    ...
                    : '${_reportLabel(warning.status.name)} at '
                          '${_dateTime.format(operationsReportPlantTime(warning.closureRequestedAt!))} by '
                ...
                warning.closedAt == null
                    ...
                    : '${_reportLabel(warning.closureDisposition?.name ?? 'closed')} at '
                          '${_dateTime.format(operationsReportPlantTime(warning.closedAt!))} by '
        ...
        rows: report.qualityMonitoringRequests
            .map(
              (request) => <String>[
                ...
                request.reason,
                '${_dateTime.format(operationsReportPlantTime(request.createdAt))} by '
                    '${request.createdByName ?? 'Not recorded'}',
                request.closedAt == null
                    ? _reportLabel(request.status.name)
                    : 'Closed ${_dateTime.format(operationsReportPlantTime(request.closedAt!))} by '
        ...
        rows: report.inspectionFindings
            .map(
              (finding) => <String>[
                ...
                '${_dateTime.format(operationsReportPlantTime(finding.firstObservedAt))}\n'
                    '${_dateTime.format(operationsReportPlantTime(finding.latestObservedAt))}',
        ...
        ..._simpleTables(
          ...
          rows: assets
              .map((asset) {
                ...
                final burnerEvidenceComplete = _burnerEvidenceComplete(round);
                final uvEvidenceComplete = _uvEvidenceComplete(round);
                final draftSealEvidenceComplete = _draftSealEvidenceComplete(
                  round,
                );
                ...
                return <String>[
                  round.assetName,
                  _roundEvidenceSummary(round),
                  !burnerEvidenceComplete
                      ? 'Unknown — incomplete burner evidence'
                      : round.redHotPositions.isEmpty
                      ? 'None recorded'
                      : round.redHotPositions
                            .map((position) => 'B$position')
                            .join(', '),
                  !draftSealEvidenceComplete
                      ? 'Unknown — incomplete evidence'
                      : draftFlags.isEmpty
                      ? 'No exception'
                      : draftFlags.join(', '),
                  !uvEvidenceComplete
                      ? 'Unknown — incomplete UV evidence'
                      : uvExceptions.isEmpty
                      ? 'All in service'
                      : uvExceptions,
                  _roundEvidenceObservers(round),
                ];
              })
              .toList(growable: false),
          widths: const <int, pw.TableColumnWidth>{
  ...
  static List<List<String>> _reportRowSegments(List<String> row) {
    ...
    return List<List<String>>.generate(
      segmentCount,
      (segmentIndex) => List<String>.generate(cells.length, (cellIndex) {
        final segments = cells[cellIndex];
        if (segmentIndex < segments.length) return segments[segmentIndex];
        // Repeat the leading identity cell on continuation rows. Long
        // narrative cells may span pages, but a continuation must not become
        // an unattributed event or physical subject in the exported table.
        if (segmentIndex > 0 && cellIndex == 0 && segments.isNotEmpty) {
          const prefix = 'continued: ';
          const maximumCellCharacters = 360;
          const maximum = maximumCellCharacters - prefix.length;
          final first = segments.first;
          return '$prefix${first.substring(0, first.length > maximum ? maximum : first.length)}';
        }
        if (segmentIndex > 0 && segments.length == 1) return segments.single;
        return '';
      }, growable: false),
      growable: false,
    );
  ...
  static String _formatLocalDate(DateTime value) =>
      _date.format(operationsReportPlantTime(value));
  ...
  static bool _burnerEvidenceComplete(BurnerConditionRound round) {
    if (round.observations.length != 8 ||
        round.observations.map((item) => item.position).toSet().length != 8) {
      return false;
    }
    return List<int>.generate(8, (index) => index + 1).every(
      (position) =>
          round.evidenceFor('burners.$position.redHotObserved').kind !=
          'unknown',
    );
  }

  static bool _uvEvidenceComplete(BurnerConditionRound round) {
    if (round.uvObservations.length != 8 ||
        round.uvObservations.map((item) => item.position).toSet().length != 8) {
      return false;
    }
    return List<int>.generate(8, (index) => index + 1).every(
      (position) =>
          round.evidenceFor('uv.$position.condition').kind != 'unknown',
    );
  }

  static bool _draftSealEvidenceComplete(BurnerConditionRound round) =>
      round.evidenceFor('draftSealRedHotObserved').kind != 'unknown' &&
      round.evidenceFor('hotAirAtDraftSealObserved').kind != 'unknown';

  static String _roundEvidenceSummary(BurnerConditionRound round) {
    final fields = <String>[
    ...
    final attributions = fields
        .map(round.evidenceFor)
        .where((evidence) => evidence.kind != 'unknown')
        .map(_evidenceAttribution)
        .toSet()
        .toList(growable: false);
    if (attributions.isEmpty) return 'Unknown — no attributable evidence';
    if (attributions.length == 1) return attributions.single;
    return 'Mixed evidence\n${attributions.join('; ')}';
  }

  static String _roundEvidenceObservers(BurnerConditionRound round) {
    final observers = <String>{
      for (var position = 1; position <= 8; position++)
        _observerAttribution(
          round.evidenceFor('burners.$position.redHotObserved'),
        ),
      for (var position = 1; position <= 8; position++)
        _observerAttribution(round.evidenceFor('uv.$position.condition')),
      _observerAttribution(round.evidenceFor('draftSealRedHotObserved')),
      _observerAttribution(round.evidenceFor('hotAirAtDraftSealObserved')),
    }..removeWhere((value) => value == 'Unknown');
    if (observers.isEmpty) return 'Unknown';
    if (observers.length == 1) return observers.single;
    return 'Mixed observers\n${observers.join('; ')}';
  }

  static String _evidenceAttribution(BurnerEvidenceProvenance evidence) {
    final observedAt = evidence.observedAt;
    if (observedAt == null) return 'Unknown';
    return _formatLocalDateTime(observedAt);
  }

  static String _observerAttribution(BurnerEvidenceProvenance evidence) =>
      evidence.observerName?.trim().isNotEmpty == true
      ? evidence.observerName!.trim()
      : 'Unknown';
  ...
  static pw.Widget _incompleteEvidenceStatement(String text) => pw.Container(
    width: double.infinity,
    padding: const pw.EdgeInsets.all(10),
    decoration: pw.BoxDecoration(
      color: const PdfColor(1, 0.96, 0.88),
      border: pw.Border.all(
        color: const PdfColor(0.70, 0.48, 0.15),
        width: 0.8,
      ),
      borderRadius: const pw.BorderRadius.all(pw.Radius.circular(4)),
    ),
  ...
  static List<String> _innerCoverConditionRow(InnerCoverProfile profile) {
    ...
    final linkage = profile.currentBaseAssetNumber == null
  ...
  static String _assetStateTime(PlantAssetState state) {
    final details = <String>[
      if (state.operationalCondition?.declaredAt != null)
        'Declared ${_dateTime.format(operationsReportPlantTime(state.operationalCondition!.declaredAt!))}',
      if (state.availability?.since != null)
        'Blocked ${_dateTime.format(operationsReportPlantTime(state.availability!.since!))}',
      if (state.workflowStatus?.lastTransitionAt != null)
        'Workflow ${_dateTime.format(operationsReportPlantTime(state.workflowStatus!.lastTransitionAt!))}',
      for (final issue in state.issueConditionContributions)
        'Issue ${issue.ticketId}: ${_dateTime.format(operationsReportPlantTime(issue.startedAt))}',
    ];
  ...
  static String _complianceEvidence(ComplianceRequestRecord request) {
    final rows = <String>[
      if (request.acknowledgedAt != null)
        'Acknowledged ${_dateTime.format(operationsReportPlantTime(request.acknowledgedAt!))} by '
            '${request.acknowledgedByName ?? 'Not recorded'}',
      if (request.compliedAt != null)
        'Complied ${_dateTime.format(operationsReportPlantTime(request.compliedAt!))} by '
            '${request.compliedByName ?? 'Not recorded'}: '
            '${request.complianceNote ?? 'No note'}',
      if (request.confirmedAt != null)
        'Confirmed ${_dateTime.format(operationsReportPlantTime(request.confirmedAt!))} by '
            '${request.confirmedByName ?? 'Not recorded'}: '
            '${request.confirmNote ?? 'No note'}',
      if (request.becameDueAt != null)
        'Due since ${_dateTime.format(operationsReportPlantTime(request.becameDueAt!))}',
  ...
  static String _ticketSubjectLabel(MaintenanceRecord ticket) {
    final reference = ticket.assetHierarchyReference;
    final cover = reference?.innerCoverAssociation;
    final subject = reference == null
        ? _assetLabel(ticket.assetType, ticket.assetNumber)
        : '${reference.assetClassName} ${ticket.assetNumber}';
    return '$subject${cover?.innerCoverId == null ? '' : '\nInner Cover ${cover!.innerCoverSerialNumber}'}\nIssue ${ticket.firestoreId ?? 'local:${ticket.id}'}';
  }

  static String _jobSubjectLabel(JobExecution job) {
    final reference = job.assignmentAssetHierarchyReference;
    final cover = job.assignmentInnerCoverPosition;
    final subject = reference == null
        ? _assetLabel(job.assetType, job.assetNumber)
        : '${reference.assetClassName} ${job.assetNumber}';
    return '$subject${cover == null ? '' : '\nInner Cover ${cover.innerCoverSerialNumber} at Base ${cover.baseAssetNumber}'}\nJob ${job.firestoreId ?? 'local:${job.id}'}';
  }
