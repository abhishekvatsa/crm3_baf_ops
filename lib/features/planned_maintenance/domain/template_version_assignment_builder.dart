...
TemplateVersionAssignmentPreview previewTemplateVersionAssignment({
  ...
  final templateName = _stringFrom(
    jobSnapshot,
    const ['jobName', 'templateName', 'title', 'name'],
  ) ??
      _clean(package.title) ??
      'Published template';
  final assetType = _parseAssetType(
    _stringFrom(
      jobSnapshot,
      const ['applicableAssetType', 'assetType', 'asset_type'],
    ) ??
        package.assetType,
  ) ??
      AssetType.base;
    ...
    modules: [
      for (var i = 0; i < moduleSnapshots.length; i++)
        TemplateVersionModulePreview(
          title: TemplateVersionSnapshotBundle.moduleTitle(moduleSnapshots[i], i),
...
PublishedTemplateAssignmentResult buildAssignmentFromPublishedTemplateVersion({
  ...
  if (packageId == null || versionId == null) {
    throw StateError('Published package/version must have Firestore IDs before assignment.');
  }
  ...
  final jobSnapshot = snapshotBundle.jobSnapshot;
  final moduleSnapshots = snapshotBundle.moduleSnapshots;
  final templateContentHash = version.contentHash ?? version.computeContentHash();
  ...
  for (var index = 0; index < moduleSnapshots.length; index++) {
    ...
    final module = JobModuleInstance()
      ...
      ..templatePackageId = packageId
      ..templateVersionId = versionId
      ..templateModuleId = _stringFrom(
        snapshot,
        const ['templateModuleId', 'moduleId', 'id', 'key'],
      )
      ..moduleCode = code
      ...
      ..assetNumber = assetNumber
      ..chargeNoAtEvent = chargeNoAtEvent
      ..pairedEquipmentJson = _jsonStringOrNull(snapshot['pairedEquipmentJson'])
      ..moduleTitle = TemplateVersionSnapshotBundle.moduleTitle(snapshot, index)
      ..moduleDescription = _stringFrom(
        snapshot,
        const ['moduleDescription', 'description', 'closedDossierOutput'],
      )
      ...
      ..isRequired = _boolFrom(snapshot, const ['isRequired', 'required'], fallback: true)
      ..requiredForClosure = _boolFrom(
        snapshot,
        const ['requiredForClosure', 'requiredForCloseout', 'required'],
        fallback: true,
      )
      ..addedDuringExecution = false
      ..displayOrder = _intFrom(snapshot, const ['displayOrder', 'order', 'sequence'], fallback: index)
      ..functionalSection = _stringFrom(snapshot, const ['functionalSection', 'section'])
      ..componentGroup = _stringFrom(snapshot, const ['componentGroup', 'component'])
      ..subsystem = _stringFrom(snapshot, const ['subsystem', 'catalogueArea', 'area'])
      ..targetRef = _stringFrom(snapshot, const ['targetRef'])
      ..targetRefs = _stringListFrom(snapshot, const ['targetRefs', 'targets'])
      ..procedureRefs = _stringListFrom(snapshot, const ['procedureRefs', 'procedures'])
      ..safetyConfirmations = _stringListFrom(snapshot, const ['safetyConfirmations'])
      ..tags = _moduleTags(package, version, snapshot, code)
      ..operationalStatePreconditions = _stringListFrom(
        snapshot,
        const ['operationalStatePreconditions', 'preconditions'],
      )
      ..responsesJson = '[]'
      ..actionsJson = '[]'
      ...
      ..addReason = 'Assigned from published TemplateVersion v${version.versionNumber}'
  ...
  if (modules.isEmpty) {
    throw StateError('Published template version has no module snapshots to assign.');
  }

  return PublishedTemplateAssignmentResult(execution: execution, modules: modules);
}

TemplateVersionSnapshotBundle _snapshotBundleForAssignment(TemplateVersion version) {
  try {
...
String _encodeList(List<Map<String, dynamic>> value) => _jsonIndent.convert(value);
...
int _intFrom(Map<String, dynamic> map, List<String> keys, {required int fallback}) {
  for (final key in keys) {
    final value = map[key];
...
bool _boolFrom(
  ...
  for (final key in keys) {
    ...
    if (value is String) {
      ...
      if (cleaned == 'true' || cleaned == 'yes' || cleaned == 'required') return true;
      if (cleaned == 'false' || cleaned == 'no' || cleaned == 'optional') return false;
    }
  }
  return fallback;
}
...
List<String> _assignedAgenciesFrom(
  ...
  final fromSnapshot = _stringListFrom(
    jobSnapshot,
    const ['assignedAgencies', 'agencies', 'disciplines', 'disciplineScope'],
  );
  ...
  return combined.isEmpty ? <String>['mechanical'] : (combined.toList()..sort());
}
