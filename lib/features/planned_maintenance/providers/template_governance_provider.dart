...
void _normalizePackageForUserSave(
  ...
  record
    ...
    ..activeVersionFirestoreId = _cleanOptionalText(record.activeVersionFirestoreId)
    ...
    _cleanStringList(record.operationalStatePreconditions)
...
void _normalizeVersionForUserSave(
  ...
  record
    ...
    ..sourceVersionFirestoreId = _cleanOptionalText(record.sourceVersionFirestoreId)
    ..contentHash = _cleanOptionalText(record.contentHash)
    ..jobTemplateSnapshotJson =
    _cleanRequiredText(record.jobTemplateSnapshotJson, '{}')
    ..moduleSnapshotsJson = _cleanRequiredText(record.moduleSnapshotsJson, '[]')
    ..fieldDefinitionsJson =
    _cleanRequiredText(record.fieldDefinitionsJson, '[]')
...
abstract class TemplateGovernanceRepository {
  ...
  Future<List<TemplateVersion>> getVersionsForPackage(String packageFirestoreId);
  Stream<List<TemplateVersion>> watchVersionsForPackage(String packageFirestoreId);
  Future<TemplateVersion?> getVersionById(dynamic id);
  Future<TemplateVersion?> getVersionByFirestoreId(String firestoreId);

  Future<List<TemplatePublishAudit>> getAuditsForVersion(String versionFirestoreId);
  ...
  Future<RemoteTombstoneApplyResult> applyTombstoneFromPackageRemote(TemplatePackage remote);
  ...
  Future<RemoteTombstoneApplyResult> applyTombstoneFromVersionRemote(TemplateVersion remote);
...
class IsarTemplateGovernanceRepository implements TemplateGovernanceRepository {
  @override
  Future<void> savePackage(TemplatePackage record, {required AppUser actor}) async {
  ...
  @override
  Future<void> saveVersion(TemplateVersion record, {required AppUser actor}) async {
      ...
      if (packageId != null) {
        ...
        if (package != null) {
          package
            ...
            ..latestVersionNumber = record.versionNumber > package.latestVersionNumber
  ...
  @override
  Future<List<TemplateVersion>> getVersionsForPackage(String packageFirestoreId) async {
  ...
  @override
  Future<List<TemplatePublishAudit>> getAuditsForVersion(String versionFirestoreId) async {
  ...
  @override
  Future<TemplatePublishAudit?> getAuditByFirestoreId(String firestoreId) async {
  ...
  Future<void> markPackagesSynced(List<int> ids) async {
    await isar.writeTxn(() async {
      final records = (await isar.templatePackages.getAll(ids)).whereType<TemplatePackage>().toList();
  ...
  Future<void> markPackagesSyncedIfUnchanged(
    ...
    await isar.writeTxn(() async {
      final records = (await isar.templatePackages.getAll(byId.keys.toList()))
          .whereType<TemplatePackage>()
          .toList();
  ...
  @override
  Future<RemoteTombstoneApplyResult> applyTombstoneFromPackageRemote(TemplatePackage remote) async {
    if (remote.firestoreId == null) return const RemoteTombstoneApplyResult.localMissing();
    if (!remote.isDeleted) return const RemoteTombstoneApplyResult.notDeletedRemote();

    return isar.writeTxn<RemoteTombstoneApplyResult>(() async {
      final local = await getPackageByFirestoreId(remote.firestoreId!);
      if (local == null) return const RemoteTombstoneApplyResult.localMissing();
      if (local.isDeleted) return RemoteTombstoneApplyResult.alreadyDeleted(local);
  ...
  Future<void> markVersionsSynced(List<int> ids) async {
    await isar.writeTxn(() async {
      final records = (await isar.templateVersions.getAll(ids)).whereType<TemplateVersion>().toList();
      for (final record in records) {
        record.isSynced = true;
      }
  ...
  Future<void> markVersionsSyncedIfUnchanged(
    ...
    await isar.writeTxn(() async {
      final records = (await isar.templateVersions.getAll(byId.keys.toList()))
  ...
  @override
  Future<RemoteTombstoneApplyResult> applyTombstoneFromVersionRemote(TemplateVersion remote) async {
    if (remote.firestoreId == null) return const RemoteTombstoneApplyResult.localMissing();
    if (!remote.isDeleted) return const RemoteTombstoneApplyResult.notDeletedRemote();
  ...
  Future<void> markAuditsSynced(List<int> ids) async {
    await isar.writeTxn(() async {
      final records = (await isar.templatePublishAudits.getAll(ids))
          .whereType<TemplatePublishAudit>()
          .toList();
      for (final record in records) {
        record.isSynced = true;
      }
  ...
  Future<void> markAuditsSyncedIfUnchanged(
    ...
    await isar.writeTxn(() async {
      final records = (await isar.templatePublishAudits.getAll(byId.keys.toList()))
          .whereType<TemplatePublishAudit>()
          .toList();
      ...
      if (unchanged.isNotEmpty) await isar.templatePublishAudits.putAll(unchanged);
    });
  }
  ...
  @override
  Future<PaginatedTemplatePackageResult> getUpdatedPackages({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    return PaginatedTemplatePackageResult(records: const [], lastDoc: null);
  }

  @override
  Future<PaginatedTemplateVersionResult> getUpdatedVersions({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    return PaginatedTemplateVersionResult(records: const [], lastDoc: null);
  }

  @override
  Future<PaginatedTemplateAuditResult> getUpdatedAudits({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    return PaginatedTemplateAuditResult(records: const [], lastDoc: null);
  }

  @override
  Future<List<TemplatePackage>> getPackagesByFirestoreIds(List<String> ids) async {
  ...
  @override
  Future<List<TemplateVersion>> getVersionsByFirestoreIds(List<String> ids) async {
    if (ids.isEmpty) return [];
...
class FirestoreTemplateGovernanceRepository implements TemplateGovernanceRepository {
  ...
  final _audits = FirebaseFirestore.instance.collection('template_publish_audits');

  @override
  Future<void> savePackage(TemplatePackage record, {required AppUser actor}) async {
    _requireTemplateGovernor(actor, 'save template packages');
    _normalizePackageForUserSave(record, actor: actor, markUnsynced: false);
    await _packages.doc(record.firestoreId).set(record.toMap(), SetOptions(merge: true));
  }
  ...
  Future<void> saveVersion(TemplateVersion record, {required AppUser actor}) async {
    ...
    await _versions.doc(record.firestoreId).set(record.toMap(), SetOptions(merge: true));
  }
  ...
  Future<void> publishVersion(
    ...
    await FirebaseFirestore.instance.runTransaction((txn) async {
      ...
      txn.set(_versions.doc(record.firestoreId), record.toMap(), SetOptions(merge: true));
      txn.set(_audits.doc(audit.firestoreId), audit.toMap(), SetOptions(merge: true));
  ...
  Future<void> retireVersion(
    ...
    await FirebaseFirestore.instance.runTransaction((txn) async {
      ...
      final isActiveVersion =
          packageSnap?.data()?['activeVersionFirestoreId'] == record.firestoreId;
  ...
  Future<void> saveAudit(TemplatePublishAudit record) async {
    ...
    record.isSynced = true;
    await _audits.doc(record.firestoreId).set(record.toMap(), SetOptions(merge: true));
  }

  @override
  Future<List<TemplatePackage>> getAllPackages() async {
    final snap = await _packages.where('isDeleted', isEqualTo: false).get();
    final records = snap.docs.map((doc) => TemplatePackage.fromMap(doc.data(), doc.id)).toList();
    records.sort((a, b) => a.title.compareTo(b.title));
    return records;
  }
  ...
  Future<List<TemplateVersion>> getVersionsForPackage(String packageFirestoreId) async {
    final snap = await _versions
        ...
        .where('isDeleted', isEqualTo: false)
        .get();
    final records = snap.docs.map((doc) => TemplateVersion.fromMap(doc.data(), doc.id)).toList();
    records.sort((a, b) => b.versionNumber.compareTo(a.versionNumber));
    return records;
  }
  ...
  Future<List<TemplatePublishAudit>> getAuditsForVersion(String versionFirestoreId) async {
    final snap = await _audits
        ...
        .get();
    final records = snap.docs.map((doc) => TemplatePublishAudit.fromMap(doc.data(), doc.id)).toList();
    records.sort((a, b) => b.performedAt.compareTo(a.performedAt));
    return records;
  }
  ...
  Future<PaginatedTemplatePackageResult> getUpdatedPackages({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    ...
    if (since != null) query = query.where('updatedAt', isGreaterThan: since.toIso8601String());
    if (startAfter != null) query = query.startAfterDocument(startAfter);
    final snap = await query.limit(limit).get();
    return PaginatedTemplatePackageResult(
      records: snap.docs.map((doc) => TemplatePackage.fromMap(doc.data(), doc.id)).toList(),
  ...
  Future<PaginatedTemplateVersionResult> getUpdatedVersions({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    ...
    return PaginatedTemplateVersionResult(
      records: snap.docs.map((doc) => TemplateVersion.fromMap(doc.data(), doc.id)).toList(),
      lastDoc: snap.docs.isNotEmpty ? snap.docs.last : null,
    );
  }
  ...
  Future<PaginatedTemplateAuditResult> getUpdatedAudits({DateTime? since, int limit = 500, DocumentSnapshot? startAfter}) async {
    ...
    return PaginatedTemplateAuditResult(
      records: snap.docs.map((doc) => TemplatePublishAudit.fromMap(doc.data(), doc.id)).toList(),
      lastDoc: snap.docs.isNotEmpty ? snap.docs.last : null,
    );
  }
  ...
  Future<List<TemplatePackage>> getPackagesByFirestoreIds(List<String> ids) async {
    ...
    for (var i = 0; i < ids.length; i += 30) {
      ...
      final snap = await _packages.where(FieldPath.documentId, whereIn: chunk).get();
      results.addAll(snap.docs.map((doc) => TemplatePackage.fromMap(doc.data(), doc.id)));
    }
    return results;
  }
  ...
  Future<void> batchUpsertPackages(List<TemplatePackage> records) async {
    ...
    for (final record in records) {
      if (record.firestoreId != null) {
        batch.set(_packages.doc(record.firestoreId), record.toMap(), SetOptions(merge: true));
      }
    }
    await batch.commit();
  }
  ...
  Future<List<TemplateVersion>> getVersionsByFirestoreIds(List<String> ids) async {
    ...
    for (var i = 0; i < ids.length; i += 30) {
      final chunk = ids.sublist(i, i + 30 > ids.length ? ids.length : i + 30);
      final snap = await _versions.where(FieldPath.documentId, whereIn: chunk).get();
      results.addAll(snap.docs.map((doc) => TemplateVersion.fromMap(doc.data(), doc.id)));
    }
    return results;
  }
  ...
  Future<void> batchUpsertVersions(List<TemplateVersion> records) async {
    ...
    for (final record in records) {
      if (record.firestoreId != null) {
        batch.set(_versions.doc(record.firestoreId), record.toMap(), SetOptions(merge: true));
      }
    }
    await batch.commit();
  }
  ...
  Future<List<TemplatePublishAudit>> getAuditsByFirestoreIds(List<String> ids) async {
    ...
    for (var i = 0; i < ids.length; i += 30) {
      final chunk = ids.sublist(i, i + 30 > ids.length ? ids.length : i + 30);
      final snap = await _audits.where(FieldPath.documentId, whereIn: chunk).get();
      results.addAll(snap.docs.map((doc) => TemplatePublishAudit.fromMap(doc.data(), doc.id)));
    }
    return results;
  }
  ...
  Future<void> batchUpsertAudits(List<TemplatePublishAudit> records) async {
    ...
    for (final record in records) {
      if (record.firestoreId != null) {
        batch.set(_audits.doc(record.firestoreId), record.toMap(), SetOptions(merge: true));
      }
    }
    await batch.commit();
  }
...
final firestoreTemplateGovernanceRepo = Provider<FirestoreTemplateGovernanceRepository>(
      (ref) => FirestoreTemplateGovernanceRepository(),
);

final templateGovernanceRepositoryProvider = Provider<TemplateGovernanceRepository>((ref) {
  return kIsWeb
...
final packageVersionsProvider = StreamProvider.family<List<TemplateVersion>, String>(
      (ref, packageFirestoreId) {
    return ref
