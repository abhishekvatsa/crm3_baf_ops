...
part 'inspection_corrective_link.dart';
part 'inspection_finding_actions.dart';
        ...
        return RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              if (!evidence.isComplete || !evidence.isServerVerified)
                Padding(
                  padding: const EdgeInsets.only(bottom: BafSpacing.lg),
                  child: Text(
                    !evidence.isComplete
                        ? 'Incomplete campaign list: ${evidence.rejectedDocumentIds.length} entries could not be read. Counts and choices cover readable campaigns only. Available campaigns can still be used.'
                        : 'Showing saved campaigns while waiting for server confirmation. The list may be incomplete.',
                  ),
                ),
              if (rows.isEmpty)
                _InspectionEmpty(
                  ...
                  title: !evidence.isComplete || !evidence.isServerVerified
                      ? 'No readable campaigns in this view'
                      : closed
        ...
        if (campaign == null) {
          return BafScreenStateScaffold(
            ...
            state: BafStatePanel.empty(
              title: evidence.isComplete && evidence.isServerVerified
                  ? 'Campaign not found'
                  : 'Campaign could not be confirmed',
              message: evidence.isComplete && evidence.isServerVerified
                  ? 'This inspection campaign is no longer available.'
                  : 'The campaign list is incomplete or awaiting the server. This does not establish that the campaign was removed.',
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (actor.canManageInspectionCampaigns &&
              campaign.status == InspectionCampaignStatus.closed)
            IconButton(
              tooltip: 'Review target after repair or relocation',
          ...
          : observationsAsync.when(
              ...
              data: (evidence) {
                ...
                final blockingFindings = findings
                    .where((finding) => !_inspectionFindingIsTerminal(finding))
                    .length;
                  ...
                  children: [
                    ...
                    if (campaign.status == InspectionCampaignStatus.closed)
                      _InlineNotice(
                        ...
                        text:
                            'Survey closed — $blockingFindings findings outstanding. Follow-up and verification continue below; the original closure remains in history.',
                      ),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (!_inspectionFindingIsTerminal(finding) &&
                                ...
                                TextButton.icon(
                                  icon: const Icon(Icons.add_chart_rounded),
                                  label: const Text('Add follow-up reading'),
                                  onPressed: () => _recordObservation(
                                    context,
                                    ref,
                                    campaign,
                                    nodes,
                                    initialTargetKey: finding.targetKey,
                                    followUpFindingId: finding.id,
                                  ),
                                ),
                              for (final reading in currentRows.where(
                                (row) => row.id == finding.currentObservationId,
                              ))
                                if (!_inspectionFindingIsTerminal(finding) &&
                                    actor.canSuperviseInspectionObservations)
                                  TextButton.icon(
                                    ...
                                    label: const Text(
                                      'Review or change corrective work',
                                    ),
                                    onPressed: () => _linkIssue(
                                      context,
                                      ref,
                                      campaign,
                                      reading,
                      ...
                      ...rows.map(
                        (observation) => Padding(
                          ...
                          child: _ObservationCard(
                            ...
                            canCorrect:
                                campaign.status !=
                                    InspectionCampaignStatus.paused &&
                                (actor.canSuperviseInspectionObservations ||
  ...
  Future<void> _openAuditPdf(BuildContext context, WidgetRef ref) async {
    try {
      ...
      final report = buildInspectionCampaignReport(
        ...
        createdFindingIds: evidence.createdFindingIds,
...
Future<WorkflowCommandReceipt?> _runInspectionCommand(
  ...
  try {
    if (originUid != null &&
        ref.read(currentAppUserProvider).value?.uid != originUid) {
      throw StateError(
        'Return to the account that opened this action. Nothing was submitted.',
      );
    }
