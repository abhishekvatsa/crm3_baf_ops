...
class AssetValidator {
  ...
  static bool isValid(AssetType type, int number) {
    switch (type) {
      case AssetType.base:
        ...
        return (number >= 101 && number <= 124) ||
            (number >= 201 && number <= 223);
      ...
      case AssetType.innerCover:
        // Inner Cover issues use the operationally familiar Base number. The
        // governed repository verifies the physical Base and linked serial.
        return number > 0;
  ...
  static String? getValidationMessage(AssetType type, int number) {
    ...
    switch (type) {
      ...
      case AssetType.innerCover:
        return 'Enter the Base number carrying the Inner Cover';
      case AssetType.governedCustom:
